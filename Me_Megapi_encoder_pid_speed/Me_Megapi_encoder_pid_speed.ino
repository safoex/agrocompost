...
void setup_motors() {
  ...
  Serial.println(Encoder_2.getPortB());
}
...
class WindowedVar {
  ...
  int filled;
  public:
  WindowedVar(): filled(0) {
    memset(window, sizeof(float)*WINDOW_SIZE, 0); 
  }
  void add(float a) {
  ...
  float add_and_get(float a) {
    this->add(a);
    return this->get();
  }
};
WindowedVar my, mx, mz, mm;
...
void loop()
{
  ...
  my.add(myIMU.my);
  mz.add(myIMU.mz);
  mm.add(sqrt(myIMU.mx*myIMU.mx + myIMU.my*myIMU.my + myIMU.mz*myIMU.mz));
//  Serial.print("mx = "); 
//  Serial.print( (int)mx.get() );
//  Serial.print(" my = "); 
  Serial.println( (int)mm.get() );
//  Serial.print(" mz = "); 
//  Serial.print( (int)mz.get() );
//  Serial.println(" mG");
  if(Serial.available())
  {
    char a = Serial.read();
    switch(a)
    {
      ...
      break;
      case '1':
      Encoder_1.runSpeed((int)base_speed);
      Encoder_2.runSpeed(-(int)base_speed);
      break;
      default:
      break;
    }
